...
class DistillationLoss(torch.nn.Module):
    ...
    def __init__(self, base_criterion: torch.nn.Module, teacher_model: torch.nn.Module,
                 keep_ratio: float, distillation_type: str, alpha: float, tau: float):
        super().__init__()
    ...
    def forward(self, inputs, outputs, labels):
        ...
        if not isinstance(outputs, torch.Tensor):
            # assume that the model outputs a tuple of [outputs, outputs_masks]
            outputs, outputs_masks = outputs
            
        base_loss = self.base_criterion(outputs, labels)
        
        pred_loss = 0
        for i, masks in enumerate(outputs_masks):
            pred_loss = pred_loss + ((masks.mean((-1, -2))-self.keep_ratio)**2).mean()
        ...
        loss = base_loss * (1 - self.alpha) + distillation_loss * self.alpha + pred_loss / len(outputs_masks)
        return loss
